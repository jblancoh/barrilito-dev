...
export interface HudStatusProps {
  ...
  canBack: boolean
  showRail: boolean
  /** Narrow viewports get a single-row card so it doesn't crowd the board and panel. */
  compact: boolean
...
export function HudStatus({
  ...
  const square = String(sq).padStart(2, "0")

  return (
    <div className={cn("absolute top-20 z-20 flex flex-col gap-3", compact ? "left-4 right-4 max-w-[340px]" : "left-4 w-[232px]")}>
      {compact ? (
        <div className="flex flex-col gap-1.5 rounded-xl border bg-card/90 px-3 py-2.5 shadow-[0_10px_30px_-12px_rgb(0_0_0/0.4)] backdrop-blur-[10px]">
          <div className="flex items-center gap-2">
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="font-mono text-[10px] tracking-[0.08em] text-muted-foreground">CASILLA {square} / 25</div>
            ...
            <button
              ...
              onClick={onForward}
              disabled={!canForward}
              className="flex h-8 shrink-0 items-center justify-center gap-1.5 rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground transition-opacity hover:bg-primary/90 disabled:opacity-40"
            >
            ...
            <button
              ...
              disabled={!canBack}
              aria-label="Regresar"
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-input bg-transparent transition-colors hover:bg-accent hover:text-accent-foreground disabled:opacity-40"
            >
              <Undo2 className="h-3.5 w-3.5" />
            </button>
          </div>
          <div className="truncate text-xs text-muted-foreground">{nextLabel}</div>
          <div className="font-mono text-[10px] text-muted-foreground">{inputHint(coarsePointer)}</div>
        </div>
      ) : (
        <div className="flex flex-col gap-3 rounded-xl border bg-card/90 p-3.5 shadow-[0_10px_30px_-12px_rgb(0_0_0/0.4)] backdrop-blur-[10px]">
          ...
          <div className="font-mono text-[11px] text-muted-foreground">{inputHint(coarsePointer)}</div>
        </div>
      )}
